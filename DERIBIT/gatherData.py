...
class GatherData():
    ...
    def setting_url(self, map_client):
        url = map_client["url"]
        self.deribitWebSocket.set_url(url)


    def start(self):
        # parsing a delegate to the thread for this function and then say to start the thread later on
        self.APIThread = Thread(target=self.deribitWebSocket.start_socket)
    ...
    def run(self, clientMap):
        ...
        newQuote = self.sortData(self.deribitWebSocket.get_list_of_data())
